...
import { FeeType } from './FeeTypeForm';
import HelpTooltip from '../../common/HelpTooltip';

interface FeeTypeListProps {
  types: FeeType[];
  onEdit: (type: FeeType) => void;
  onDelete: (typeId: string) => void;
  loading?: boolean;
  error?: string | null;
}

const FeeTypeList: React.FC<FeeTypeListProps> = ({
  types,
  onEdit,
  onDelete,
  loading,
  ...
  if (loading) {
    return <p className="text-center text-gray-500 py-4 text-sm md:text-base">Loading fee types...</p>;
  }
  ...
  if (types.length === 0) {
    return <p className="text-center text-gray-500 py-4 text-sm md:text-base">No fee types found. Add one to get started!</p>;
  }
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th scope="col" className="hidden sm:table-cell px-4 py-2 sm:px-6 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Default Amount</th>
          ...
          {types.map((type) => (
            <tr key={type.id} className="hover:bg-gray-50">
              <td className="px-4 py-3 sm:px-6 sm:py-4 whitespace-nowrap">
                ...
                <div className="text-xs text-gray-500 truncate max-w-xs hover:whitespace-normal">{type.description || ''}</div>
              </td>
              <td className="hidden sm:table-cell px-4 py-3 sm:px-6 sm:py-4 whitespace-nowrap text-sm text-gray-500">
                {Number(type.default_amount).toFixed(2)}
              </td>
              ...
              <td className="px-4 py-3 sm:px-6 sm:py-4 whitespace-nowrap text-right text-sm font-medium">
                <div className="flex items-center justify-end space-x-1">
                  ...
                  <HelpTooltip helpKey="feeTypeList_editButton" position="left" className="inline-flex" />
                </div>
                <div className="flex items-center justify-end space-x-1 mt-1">
                  ...
                  <HelpTooltip helpKey="feeTypeList_deleteButton" position="left" className="inline-flex" />
                </div>
              </td>
            </tr>
          ))}
